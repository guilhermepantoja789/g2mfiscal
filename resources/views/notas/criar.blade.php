<x-app-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 2. CÁLCULO REVERSO (Percentual <-> Valor)
            ...
            document.querySelectorAll('.calc-tax-percent').forEach(input => {
                input.addEventListener('change', function() {
                    ...
                    if (valorServico > 0) {
                        const calculatedValue = (valorServico * percent) / 100;
                        targetInput.value = formatFloat(calculatedValue);
                    }
                });
            });

            // Digitou Valor -> Calcula %
            document.querySelectorAll('.calc-tax-value').forEach(input => {
                input.addEventListener('change', function() {
                    ...
                    if (valorServico > 0) {
                        const calculatedPercent = (valorImposto / valorServico) * 100;
                        targetInput.value = formatFloat(calculatedPercent);
                    }
                });
            });

            // Atualiza impostos ao mudar o Valor Total
            inputValorServico.addEventListener('input', function() {
                if(getFloat(this.value) > 0) {
                    document.querySelectorAll('.calc-tax-percent').forEach(el => {
                        // Se já tiver uma porcentagem definida, recalcula o valor em reais
